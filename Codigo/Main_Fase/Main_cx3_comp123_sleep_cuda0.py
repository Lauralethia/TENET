...
if UsarTF or UsarFase:
    s_Forward = torch.cat([data2spectrum(sp,UsarTF,UsarFase) for sp in compnames(datapath + filename,NC)],dim=1) # NC tiene las componentes usadas
    s_Backward = torch.cat([data2spectrum(sp,UsarTF,UsarFase) for sp in compnames(datapath + filename_reversal,NC)],dim=1)
