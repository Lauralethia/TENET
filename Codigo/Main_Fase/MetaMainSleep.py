...
#%%#############################################################################
# Cosas para editar
PLACA = "cuda:1" ## En que placa laburo? o "cuda:1"/"cuda:0" y ver lo de paralelizar
...
#asfunction_corridaSimple(datapath,resultpath,filename,3, [1,2,3],True,True,PLACA) 

#asfunction_corridaSimple(datapath,resultpath,filename,model_complexity = 3,NC = [1,2,3],UsarTF = True,UsarFase = True,PLACA) 
for model_complexity in [1,2,3]: # Complejidad del modelo, puede ser 3 (5 bloques), 2 (3 bloques) o 1 (1 Bloque)
    for i,NC in enumerate(Ncomp):
        for i,tf_cond in enumerate(TF_conditions):
            ...
            asfunction_corridaSimple(datapath,resultpath,filename,model_complexity,NC,UsarTF,UsarFase,PLACA) 